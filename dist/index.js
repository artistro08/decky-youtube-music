    ...
    return (SP_JSX.jsx(Section, { children: queue.map((item, index) => {
            ...
            if (DFL.DialogButton) {
                return (SP_JSX.jsxs(DFL.Focusable, { style: { display: 'flex', alignItems: 'stretch', marginTop: '2px', marginBottom: '2px' }, "flow-children": "horizontal", children: [SP_JSX.jsxs(DFL.DialogButton, { className: isSelected ? 'yt-queue-active' : undefined, style: {
                                ...
                                borderRadius: '0',
                                overflow: 'hidden',
                            }, onClick: () => { void handleJump(index); }, children: [SP_JSX.jsxs("div", { style: { width: '44px', height: '44px', flexShrink: 0, alignSelf: 'center', position: 'relative', background: 'rgba(255,255,255,0.05)' }, children: [r?.thumbnail?.thumbnails?.[0]?.url ? (SP_JSX.jsx("img", { src: r.thumbnail.thumbnails[0].url, alt: "", style: { width: '44px', height: '44px', objectFit: 'cover', display: 'block' } })) : (SP_JSX.jsx("div", { style: { width: '44px', height: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gpSystemLighterGrey)' }, children: SP_JSX.jsx(FaMusic, { size: 18 }) })), isSelected && (SP_JSX.jsx("div", { style: {
